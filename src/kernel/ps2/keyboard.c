...
void ps2_init_keyboards()
{
    ...
    if (PS2_DEVICE_1_KB)
    {
        ...
        if (ps2_kb_1_scancode_set == 0xff)
            printf("Unsupported scancode set for keyboard 1\n");
    }

    if (PS2_DEVICE_2_KB)
    {
        LOG(DEBUG, "Keyboard 2 scancode set : %u", ps2_kb_2_scancode_set);
        if (ps2_kb_2_scancode_set == 0xff)
            printf("Unsupported scancode set for keyboard 2\n");
    }
}
